...
# 4. Print all the IPs to the screen. Start:
from time import sleep def IP_LIST():
    list_ip=[]
    for i in range(6):
        list_ip.append(input("Enter a IP: "))
    return list_ip def menu():
    while(True):
...
to the list\n3.Delete Ip from the list\n4.Print all IP!\n")
        ...
        if(input("\nDo you want to quit? y/n\n")=="y"):
            break
    print("\nThanks and bye bye...") def search(a):
    sleep(1)
    if a in my_list:
        print("It is here!!")
        return "yes"
        ...
        print("It's not here!")
        return "no" def append(a):
    my_list.append(a)
    sleep(1)
    print("Your IP address has been added to the list!") def delete():
    del_ip=input("Delete ip Address:")
    my_list.remove(del_ip)
    print(my_list) def printer():
    print("Printing all IP.....")
    print(my_list) my_list=IP_LIST() print("My new list is: " + 
str(my_list))
menu()
